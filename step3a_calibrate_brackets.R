...
report <- function(label, cal) {
  ...
  if (!is.na(cal$c_star)) {
    if (isTRUE(cal$boundary_hit))
      cat("  *** WARNING: c* sits at the grid boundary -- NOT a true interpolated\n",
          "      value. Extend c_grid past it and re-run.\n", sep = "")
...
saveRDS(list(bhm = c_bhm, no_borrow = cal_nb$c_star, pool = cal_pool$c_star,
             ...
             boundary_hit = list(no_borrow = cal_nb$boundary_hit,
                                 pool = cal_pool$boundary_hit),
