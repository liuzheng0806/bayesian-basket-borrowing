## ============================================================================
## RESCUE RUN (auto-merge): re-run the funnel-prone EXNEX configs at
## adapt_delta=0.999 / max_tree=15, then AUTOMATICALLY splice the re-run configs
## back into the existing prior_sensitivity_results.rds, keeping all 12 configs.
##
## YOU DO NOTHING MANUAL. Just source this file. It:
##   1. reads the current prior_sensitivity_results.rds (your full 12-config result),
##   2. re-runs only the RUN_ONLY configs (the EXNEX funnel ones) at 0.999,
##   3. writes back a COMPLETE rds = re-run configs + the untouched others.
##
## You already backed up the rds -- good. The script reads the current file into
## memory at the START (before it overwrites anything), so the merge is safe even
## though it writes to the same filename.
##
## (.rds = one file holding one R object: here a list with the OC table $PS and the
##  divergence table $DIV. readRDS() loads it; saveRDS() writes it. "Merge" just means
##  replace the re-run configs' rows and keep the rest -- the script does this for you.)
## ============================================================================
...
## ---- run controls --------------------------------------------------------
DRY_RUN  <- FALSE        # TRUE -> M=50, 2 chains: validate the code path locally, fast.
                        # set FALSE for the cluster run (M=2000, paired with main analysis).
RUN_ONLY <- c("exnex_tau_hn_s0.5","exnex_tau_hn_s2","exnex_tau_hcauchy",
             "exnex_pi_0.3","exnex_nex_logit_p0")  # RESCUE: re-run only the EXNEX funnel configs at 0.999.
#  (To make the merged rds EXACTLY match the FINAL script, set this to ALL 8 EXNEX configs,
#   adding "exnex_tau_invgamma","exnex_pi_0.7","exnex_pi_beta11". The 5 above are the worst.)
# RUN_ONLY <- NULL        # original: all configs; or a character vector of labels to subset,
...
## ---- run all configs -----------------------------------------------------
## snapshot the existing FULL results (all 12 configs) BEFORE we overwrite the file,
## so we can splice the re-run configs back in and keep the saved rds COMPLETE.
PREV <- tryCatch(readRDS("prior_sensitivity_results.rds"), error = function(e) NULL)
if (is.null(PREV)) cat("WARNING: no existing prior_sensitivity_results.rds found to merge into.\n")
...
## ---- AUTO-MERGE: keep the previously-run configs that we did NOT re-run, and
##      splice in the freshly re-run ones, so the saved rds stays COMPLETE (all 12).
##      You do NOT need to do anything manual -- this writes the full result.
redo <- unique(PS$config)                                   # configs re-run this time
if (!is.null(PREV) && !is.null(PREV$PS)) {
  PS  <- rbind(PREV$PS [!PREV$PS$config  %in% redo, ], PS)   # kept old + new re-run
  DIV <- rbind(PREV$DIV[!PREV$DIV$config %in% redo, ], DIV)
  cat(sprintf("\nMERGED: re-ran {%s}; kept %d other configs from the previous run.\n",
              paste(redo, collapse=", "),
              length(setdiff(unique(PREV$PS$config), redo))))
} else {
  cat("\nNO PREVIOUS RESULTS MERGED -- saving ONLY the re-run configs (incomplete!).\n")
}
saveRDS(list(PS = PS, DIV = DIV,
             configs = if (!is.null(PREV)) PREV$configs else configs,
             M_calib = M_calib, M_sweep = M_sweep, dry_run = DRY_RUN),
        file = "prior_sensitivity_results.rds")
cat("saved prior_sensitivity_results.rds (COMPLETE 12-config set; re-run merged in)\n")
...
for (lab in unique(PS$config)) {           # full merged set (re-run + kept)
